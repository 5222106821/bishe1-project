...
<template>
  <div class="all">
    <el-table :data="pagination.records" border>
      ...
      <el-table-column prop="institute" label="所属部门" width="200"></el-table-column>
      ...
      <el-table-column fixed="right" label="操作" width="260">
        <template slot-scope="scope">
          <el-button @click="checkGrade(scope.row.teacherId)" type="primary"> 编辑</el-button>
          <el-button @click="deleteById(scope.row.teacherId)" type="danger"> 删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 编辑对话框-->
    <el-dialog title="编辑部门管理员信息" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
...
export default {
  ...
  methods: {
    getDepartmentManagerList() {
      //分页查询所有试卷信息
      this.$axios(`/api/departmentManagers/${this.pagination.current}/${this.pagination.size}`).then(res => {
    ...
    checkGrade(teacherId) { //修改部门管理员信息
      this.dialogVisible = true
      this.$axios(`/api/departmentManager/${teacherId}`).then(res => {
        this.form = res.data.data
      })
    },
    deleteById(teacherId) { //删除当前部门管理员
      this.$confirm("确定删除当前部门管理员吗？删除后无法恢复", "Warning", {
        confirmButtonText: '确定删除',
        cancelButtonText: '取消',
        type: 'danger'
      }).then(() => { //确认删除
        this.$axios({
          url: `/api/departmentManager/${teacherId}`,
          method: 'delete',
        }).then(res => {
          this.getDepartmentManagerList()
        })
