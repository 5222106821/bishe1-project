...
export default {
  ...
  methods: {
    ...
    addData() {
      ...
      if(role == 0) {
        const hasDeptRoleMenu = this.menu.some(item => item.index === '5')
        if (hasDeptRoleMenu) return
        this.menu.push({
          index: '5',
          title: '角色成员管理（部门管理员）',
          icon: 'icon-r-user3',
          content:[{item1:'角色成员管理（部门管理员）',path:'/departmentManagerManage',icon:"el-icon-a-041"},{item2: '添加部门管理员',path: '/addDepartmentManager',icon:"iconfont icon-r-add"}],
        })
      }
    }
  },
